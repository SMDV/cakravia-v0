...
import { AiKnowledgeQuestionSet, AiKnowledgeTest, AiKnowledgeAnswer } from '@/lib/types';
import { AiKnowledgeTestProgressManager, AiKnowledgeTestProgress } from '@/lib/aiKnowledgeTestProgress';
...
const AiKnowledgeTestInterface = () => {
  ...
  const addLog = useCallback((message: string) => {
    ...
    console.log(`AI Knowledge Test: ${message}`);
  }, []);
  ...
  const saveProgressToStorage = useCallback(() => {
    ...
    const progress: AiKnowledgeTestProgress = {
      testId: testState.test.id,
      userId: user.id,
      questionSetId: testState.questionSet?.id || '',
      questionSetName: testState.questionSet?.name || 'AI Knowledge Assessment',
    ...
    AiKnowledgeTestProgressManager.saveProgress(progress);
  ...
  const handleContinueWithoutProgress = useCallback(async () => {
    ...
    try {
      const testResponse = await aiKnowledgeAPI.getTest(pendingTestId);
      ...
      const questionSet: AiKnowledgeQuestionSet = {
        id: '',
        version: 1,
        name: 'AI Knowledge Assessment',
        active: true,
      ...
      addLog('🔄 Started AI Knowledge test without saved progress');

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to start test';
      addLog(`❌ Failed to start AI Knowledge test: ${errorMessage}`);
      setTestState(prev => ({
        ...prev,
        step: 'error',
  ...
  const handleStartNewTest = useCallback(() => {
    setShowCrossDeviceWarning(false);
    setPendingTestId(null);
    AiKnowledgeTestProgressManager.clearProgress();
    window.location.href = '/ai-knowledge-test';
  }, []);
  ...
  const initializeTest = useCallback(async () => {
    if (!isAuthenticated) {
      setTestState(prev => ({
        ...prev,
        step: 'error',
        error: 'Please login to start the AI Knowledge test'
      }));
      return;
    }

    try {
      addLog('🚀 Starting AI Knowledge test initialization...');
      ...
      if (resumeTestId) {
        ...
        const savedProgress = AiKnowledgeTestProgressManager.loadProgress(resumeTestId);

        if (!savedProgress) {
          // No saved progress found - show cross-device warning
          addLog(`⚠️ No saved progress found for AI Knowledge test: ${resumeTestId}`);
        ...
        addLog(`🔄 Resuming AI Knowledge test from localStorage: ${resumeTestId}`);

        try {
          const testResponse = await aiKnowledgeAPI.getTest(resumeTestId);
          const test = testResponse.data;
          ...
          addLog(`✅ AI Knowledge test resumed from question ${savedProgress.currentQuestionIndex + 1}`);
          return;

        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
          addLog(`⚠️ Saved AI Knowledge test is no longer valid: ${errorMessage}`);
          AiKnowledgeTestProgressManager.clearProgress();
          setTestState(prev => ({
            ...prev,
            step: 'error',
      ...
      // Check for existing progress when visiting /ai-knowledge-test directly
      const existingProgress = AiKnowledgeTestProgressManager.loadProgress();
      if (existingProgress && !existingProgress.isCompleted) {
        addLog(`ℹ️ Found existing AI Knowledge progress for test: ${existingProgress.testId}`);
        window.location.href = `/ai-knowledge-test?resumeTestId=${existingProgress.testId}`;
        return;
      }
      ...
      addLog('🎯 New AI Knowledge test ready to start!');

    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to initialize AI Knowledge test';
      addLog(`❌ AI Knowledge test initialization failed: ${errorMessage}`);
      setTestState(prev => ({
        ...prev,
        step: 'error',
        error: errorMessage
  ...
  const startTest = useCallback(() => {
    ...
    addLog('▶️ Starting AI Knowledge test...');
  ...
  const handleNextQuestion = useCallback(async () => {
    ...
    const answer: AiKnowledgeAnswer = {
    ...
    addLog(`📝 Answered AI Knowledge question ${currentQuestionIndex + 1}: ${currentSliderValue.toFixed(1)}/${maxWeight} (${answer.point}/${maxWeight} points)`);
    ...
    if (nextIndex < test.questions.length) {
      ...
      addLog('🏁 All AI Knowledge questions answered, submitting...');
      setTestState(prev => ({ ...prev, step: 'submitting' }));

      try {
        const submission = { answers: Object.values(newAnswers) };
        addLog(`📊 Submitting ${submission.answers.length} AI Knowledge answers`);
        await aiKnowledgeAPI.submitAnswers(test.id, submission);

        addLog('✅ AI Knowledge answers submitted successfully!');
        ...
        setChatHistory(prev => [...prev, {
          sender: "ai",
          type: "text",
          text: "🎉 Congratulations! You have completed the AI Knowledge Assessment. Your results are being processed..."
        }]);

        // Redirect to results page after a short delay
        setTimeout(() => {
          window.location.href = `/ai-knowledge-test-results?testId=${test.id}`;
        }, 3000);

      } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Failed to submit AI Knowledge answers';
        addLog(`❌ AI Knowledge submission failed: ${errorMessage}`);
        setTestState(prev => ({
          ...prev,
          step: 'error',
          error: errorMessage
      ...
      setTestState(prev => {
        if (prev.timeLeft <= 1) {
          ...
          if (test) {
            ...
            aiKnowledgeAPI.submitAnswers(test.id, submission)
              .then(() => {
                addLog('✅ Time up - AI Knowledge answers submitted successfully!');
              ...
              .catch((error) => {
                const errorMessage = error instanceof Error ? error.message : 'Failed to submit AI Knowledge answers';
                addLog(`❌ Time up AI Knowledge submission failed: ${errorMessage}`);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center bg-white rounded-lg shadow-lg p-8 max-w-md">
          ...
          <p className="text-gray-600 mb-4">Please login to start the AI Knowledge test</p>
          <Link
            href="/login"
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden relative" style={{ fontFamily: 'Merriweather Sans, sans-serif' }}>
      ...
      <div className="relative z-10 flex-shrink-0">
        <Header currentPage="ai-knowledge-test" transparent />
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col px-4 sm:px-6 py-4 max-w-6xl mx-auto w-full min-h-0">
        ...
        {testState.step === 'loading' && (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-6 sm:p-8 text-center">
              ...
              <h2 className="text-lg sm:text-xl font-semibold mb-2">Preparing Your AI Knowledge Test...</h2>
              <p className="text-gray-600 text-sm sm:text-base">Setting up your AI Knowledge Assessment</p>
            </div>
          </div>
        )}
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-6 sm:p-8 text-center">
              ...
              <p className="text-gray-600 mb-6 text-sm sm:text-base leading-relaxed">
                You&apos;re about to take the AI Knowledge Assessment with{' '}
                ...
                <br className="hidden sm:block" />
                This will help you understand your attitudes toward AI usage in learning.
              </p>
              <button
                onClick={startTest}
        ...
        {testState.step === 'submitting' && (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-6 sm:p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
              <h2 className="text-lg sm:text-xl font-semibold mb-2">Processing Your AI Knowledge Results...</h2>
              <p className="text-gray-600 text-sm sm:text-base">Please wait while we analyze your AI learning attitudes.</p>
            </div>
          </div>
        )}
        ...
        {testState.step === 'completed' && (
          <div className="flex-1 flex items-center justify-center">
            <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg p-6 sm:p-8 text-center">
              ...
              <h2 className="text-xl sm:text-2xl font-bold mb-4 text-green-700">AI Knowledge Assessment Completed!</h2>
              <p className="text-gray-600 mb-6 text-sm sm:text-base">
                Your AI Knowledge assessment has been submitted successfully. Redirecting to your results...
              </p>
            </div>
          </div>
        )}
...
export default AiKnowledgeTestInterface;